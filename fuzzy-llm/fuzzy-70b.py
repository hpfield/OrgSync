"""
torchrun --nproc_per_node=3 script.py
"""
...
def main():
    ...
    # Set the default CUDA device based on LOCAL_RANK
    ...
    # Broadcast uk_data to all processes
    uk_data = dist.broadcast_object_list([uk_data], src=0)[0]
    ...
    # Save grouped names to file (only by rank 0)
    if dist.get_rank() == 0:
...
"""
    ...
    for idx, (unique_name, matched_names) in enumerate(grouped_names.items()):
        # For progress display (only by rank 0)
        if dist.get_rank() == 0 and idx % 10 == 0:
            print(f"Processing group {idx+1}/{num_groups}")
    ...
    # After processing all groups, print stats (only by rank 0)
    if dist.get_rank() == 0:
...
"""
    ...
    for idx, group_set in enumerate(merged_groups):
        # For progress display (only by rank 0)
        if dist.get_rank() == 0 and idx % 10 == 0:
            print(f"Processing combined group {idx+1}/{len(merged_groups)}")

...
with tqdm(total=len(merged_groups), desc='Processing combined groups with LLM') as pbar:
    for group_set in merged_groups:
        ...
        if certainty.lower() == 'sure':
            ...
            # Store the group for further processing
...
# Function to perform web search with retries
def perform_web_search(names, num_results=3, max_retries=3):
    ...
    for name in names:
        ...
        while retries < max_retries:
            try:
                # Perform a web search for the organisation name
                query = f'"{name}"'
                ...
                if "Too Many Requests" in str(e):
                    ...
                    time.sleep(60 * retries)  # Exponential backoff: wait longer with each retry
                else:
...
# Function to process unsure groups with LLM using web search results
def process_unsure_group_with_llm(group_names, web_search_results, generator):
    ...
    # Build the web search results string
    web_results_str = ""
...
with tqdm(total=len(unsure_groups), desc='Processing unsure groups with LLM using web search') as pbar:
    for group in unsure_groups:
        ...
        # Add the result to final_groups
        final_groups.append({
            'selected_names': selected_names,
            'representative_name': representative_name
        })

# Update final_group_sizes
final_group_sizes = [len(group['selected_names']) for group in final_groups]
average_final_group_size = sum(final_group_sizes) / len(final_groups) if final_groups else 0

print(f"Total number of final groups after processing unsure groups: {len(final_groups)}")
print(f"Updated average final group size: {average_final_group_size:.2f}")
